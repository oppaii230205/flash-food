...
import { Menu, X, Leaf, ChevronDown, LogOut, User, ShoppingBag } from 'lucide-react'
        ...
        <div className="max-w-6xl mx-auto px-5 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-3">
            {isAuthenticated ? (
              <div className="relative">
                ...
                <AnimatePresence>
                  {userMenuOpen && (
                    ...
                    >
                      <Link to="/profile" className="flex items-center gap-2.5 px-4 py-2 text-sm text-green-800 hover:bg-green-50 font-semibold">
                        <User size={15} /> My Profile
                      </Link>
                      <Link to="/orders" className="flex items-center gap-2.5 px-4 py-2 text-sm text-green-800 hover:bg-green-50 font-semibold">
              ...
              <>
                <Button variant="secondary" size="sm" onClick={onLoginClick}>Sign In</Button>
                <Button size="sm" onClick={onSignupClick}>Join Free ↗</Button>
              </>
            )}
          </div>
              ...
              {isAuthenticated ? (
                <>
                  <Link to="/orders" className="flex-1">
                    <Button variant="secondary" size="sm" className="w-full">My Orders</Button>
                  </Link>
                  <Button size="sm" variant="ghost" onClick={logout} className="flex-1">Sign Out</Button>
                </>
              ) : (
                <>
                  <Button variant="secondary" size="sm" className="flex-1" onClick={() => { setMenuOpen(false); onLoginClick?.() }}>Sign In</Button>
                  <Button size="sm" className="flex-1" onClick={() => { setMenuOpen(false); onSignupClick?.() }}>Join Free ↗</Button>
                </>
              )}
            </div>
          </motion.div>
