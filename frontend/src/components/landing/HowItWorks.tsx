        ...
        <div className="text-center mb-14">
          ...
          >
            ...
            <p className="text-green-700/60 text-sm mt-2 max-w-md mx-auto">
              From discovery to pickup in under 3 minutes. No subscriptions, no waste, no fuss.
            </p>
          </motion.div>
        </div>
        ...
        <div className="relative grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          {steps.map((step, i) => (
            <motion.div
              ...
              whileHover={{ y: -4, boxShadow: '0 4px 20px rgba(52,120,80,.13)' }}
            ...
            >
              ...
              <div className={`w-[72px] h-[72px] ${step.bg} rounded-md grid place-items-center text-4xl`}>
                {step.emoji}
              </div>
              <h3 className="text-base font-extrabold text-green-900">{step.title}</h3>
              <p className="text-[0.86rem] text-green-700/60 leading-relaxed">{step.desc}</p>
            </motion.div>
          ))}
        </div>
      </div>
