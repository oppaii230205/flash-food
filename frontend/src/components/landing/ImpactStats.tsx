...
const STATS: {
  ...
  {
    icon: <BowlFood size={32} weight="duotone" className="text-green-300" />,
    target: 148400,
    suffix: "+",
    label: "Meals Rescued",
    format: "abbr",
  ...
  {
    icon: <Plant size={32} weight="duotone" className="text-green-300" />,
    target: 103,
    suffix: " tons",
  ...
  {
    icon: <Storefront size={32} weight="duotone" className="text-green-300" />,
    target: 340,
    suffix: "+",
  ...
  {
    icon: (
      <CurrencyDollar size={32} weight="duotone" className="text-green-300" />
    ),
    target: 620000,
    prefix: "$",
    suffix: "",
    label: "Saved by Rescuers",
    format: "abbr",
  },
];
...
function StatCard({
  ...
  return (
    <div className="bg-white/7 border border-white/12 rounded-lg p-7 text-center backdrop-blur-sm hover:bg-white/11 transition-all hover:-translate-y-1 duration-200">
      ...
      <div className="text-[0.78rem] text-green-300 font-medium">
        {stat.label}
      </div>
    </div>
  );
...
export function ImpactStats() {
  ...
  return (
    ...
    >
      {/* Background orbs */}
      <div
        className="absolute w-[700px] h-[700px] -top-48 -left-48 rounded-full pointer-events-none"
        style={{
          background:
            "radial-gradient(circle,rgba(82,183,136,.12) 0%,transparent 65%)",
        }}
      />
      <div
        className="absolute w-[400px] h-[400px] -bottom-24 -right-24 rounded-full pointer-events-none"
        style={{
          background:
            "radial-gradient(circle,rgba(128,222,217,.10) 0%,transparent 65%)",
        }}
      />
      ...
      <div className="max-w-6xl mx-auto px-5 relative z-10">
        <div className="text-center mb-14">
          ...
          >
            <SectionLabel dark>
              <Globe size={12} weight="fill" /> Real Numbers, Real Change
            </SectionLabel>
            ...
            <p className="text-green-300 text-sm mt-2 max-w-md mx-auto">
              Every rescue reduces methane emissions, supports local businesses,
              and feeds families in need.
            </p>
          </motion.div>
        </div>
        ...
        >
          <h3 className="text-[0.88rem] font-bold text-green-200 mb-5 flex items-center gap-2">
            <ChartBar size={15} weight="bold" /> Environmental Breakdown
          </h3>
